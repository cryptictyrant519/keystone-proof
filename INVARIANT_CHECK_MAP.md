...
| I-04 | Chain Continuity | `genesis_chain_hash`; stage-manifest `previous_chain_hash` and `chain_hash`; `stage_order`; verdict `final_chain_hash`; envelope `final_proof` | Every stage must connect to the preceding stage in the declared order, and the final proof must agree with the verdict's final chain hash. | A broken, missing, reordered, or mismatched link blocks verification. | Scorecard and Falco tampered-envelope proofs are rejected because `final_proof` does not match the verdict final chain hash. | Partially demonstrated — skipped, reordered, and duplicated-stage mutations remain pending |
...
| I-06 | Explicit Evidence Boundaries | Stage-manifest `inputs` and `outputs`; envelope `stage_manifest_refs`; `verdict_ref` | Verification evaluates only declared evidence references and does not convert undeclared environment state into verified fact. | Evidence outside the declared boundary remains unverified; missing declared evidence blocks the claim. | Threat-model boundary and proof-package contract; missing-verdict rejection. | Contract-defined — dedicated changed-input and undeclared-input mutations remain pending |
...
1. Remove each required artifact class.
2. Reorder declared stages.
3. Substitute an artifact from another run.
4. Duplicate a stage or manifest reference.
5. Change a declared input.
6. Change a declared output.
7. Alter schema or contract versions.
8. Mix artifacts from separate valid runs.
9. Replay under an incompatible environment.
10. Confirm the untouched reference proof remains `SAFE` after every disposable mutation.
